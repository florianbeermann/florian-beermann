...
Without a Web3Forms key the contact form explicitly offers "Continue in email".
It opens a pre-filled draft to `hello@florianbeermann.com` and explains that the
visitor must send that email themselves.
...
| Path | Purpose |
| --- | --- |
| `src/pages/Home.tsx` | The single-page site: brand intro, positioning, proof, engagements, transition decisions, about, contact |
| `src/components/BrandIntro.tsx`, `BrandIntro.css` | Full-viewport signature-blue intro, shared wordmark and centred scroll cue |
| `src/lib/gradient-background.ts`, `gradient-field.ts` | Native-resolution gradient renderer, fallback rendering and independent periodic motion |
...
The new first section, `#intro`, fills the viewport with five equal-width
signature-blue gradient columns and the existing Lastica/anvil wordmark. The
masthead stays hidden over the intro and returns over the content below. A small
bottom-centred chevron links to the original `#top` address,
whose unchanged positioning copy now uses the same flat paper, ink and blue
styling as the other content sections. No mountain video, poster or cloud-colour
effect is mounted or requested. All later section addresses remain unchanged.
...
`--gradient-ice`. Blue defaults to the shared `--p-blue` (`#0047ff`); ink and ice
...
does not reference or download them. They are not the new intro's fallback;
the intro renders its own static gradients without media requests.
