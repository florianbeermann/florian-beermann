...
The hero starts immediately over `public/hero-poster.jpg`. Optional desktop and
mobile videos play automatically without visible playback controls and respect
motion and data preferences. To make a
new loop from original footage, use `node scripts/build-hero-loop.mjs source.mp4`
with a local video encoder available as `ffmpeg`. The committed cuts retain the
original quality, with widths of 2560 and 1920 pixels and the source frame rate.
When replacing either file, update its version query in `Home.tsx` so visitors
do not keep an older cached copy.
