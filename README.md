...
The hero portrait is `public/portrait-plate.jpg`, built by `npm run portrait`
from `scripts/assets/portrait-source.jpg`. The generator prints the photograph
in one ink — luminance remapped onto a ramp from the page's plum to a light tint
of the same hue — and changes nothing else: the frame is the original,
background included. Tuning constants sit at the top of
`scripts/generate-portrait-plate.mjs`.

The source must be a black-and-white master. This step is a tint, not a
conversion; regrading a colour original here would put the photographer's
decisions in a build script. To change the portrait, replace the source JPEG,
re-run the script and commit the regenerated image.
...
ink-on-paper cut is what the share card is built from.
